...
assert(api.includes('loadVWorkAccountDirectory(auth)') && api.includes('accountDirectory.filter((item) => item.active && item.roles.length)'), 'The full account directory must stay separate from the active assignment directory.');
...
assert(api.includes('loadTeamDirectory(auth)') && api.includes('accounts.filter((item) => item.active && item.roles.length)') && api.includes("roles.includes('manager') ? 'manager' : roles.includes('member') ? 'member' : roles[0]"), 'Assignment options must preserve active multi-role identities from the VWork directory.');

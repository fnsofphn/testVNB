...
const taskDetailLayer = read('src/modules/vplanning/trainingOperations/TaskDetailLayer.tsx');
const detailedInputUi = read('src/modules/vplanning/trainingOperations/DetailedInputUI.tsx');
...
const activeTasksView = page.slice(page.indexOf('function Tasks('), page.indexOf('function LegacyTasks('));
...
assert(page.includes('Gán vai trò theo khóa học') && page.includes('Gỡ khỏi vai trò') && page.includes('+ Thêm tài khoản') && page.includes('Thêm khóa học') && page.includes('CopyPlus') && page.includes('Nhân bản khóa'), 'Course administration must expose the reference role matrix with removable memberships and retain add/duplicate actions.');
...
assert(page.includes('function roleCanAccessTab') && page.includes("return role === 'operations' && ['team', 'accounts', 'audit'].includes(tab)") && page.includes('nextTab !== requestedTab'), 'Only Operations may open management routes; a stale or direct forbidden URL must return other roles to Overview.');
...
assert(page.includes("!['overview', 'structure', 'team'].includes(tab)") && page.includes('className="training-context-bar"') && page.includes('layer-course-entry'), 'The overview, structure, and course-role screens must use their focused layouts instead of duplicating the global context selectors.');
assert(page.includes('TỔNG QUAN VẬN HÀNH') && page.includes('overview-kpis') && page.includes('overview-tree-table') && page.includes('Dự án › Khóa học › Lớp'), 'The default overview must preserve the reference KPI and expandable Project → Course → Class hierarchy.');
assert(page.includes('<th>Cấu trúc</th>') && page.includes('<FolderKanban') && page.includes('<BookOpen') && page.includes('<UsersRound') && !page.includes('{projectIndex + 1}.{courseIndex + 1}'), 'The overview hierarchy must use visual tree branches instead of dense decimal numbering.');
assert(page.includes('overview-class-total') && page.includes('overview-tree-title') && page.includes('{projectClasses.length} lớp') && page.includes('{courseClasses.length} lớp'), 'The overview must expose the total class count and per-branch class counts at a glance.');
assert(page.includes('overview-kpi-drilldown') && page.includes('onOpenDue={navigateDue}') && page.includes("routeContext.scope === 'all'") && page.includes('PHẠM VI TỪ TỔNG QUAN'), 'Overdue totals must drill into a refresh-safe due list with the same all/project/course/class scope.');
assert(!page.includes('className="sidebar-note"') && !page.includes('VẬN HÀNH ĐÀO TẠO'), 'The removed training sync/logout note must not return to the sidebar.');
assert(page.includes('Cấu hình công việc của lớp') && page.includes('Thay đổi được lưu ngay và ghi nhận trong Audit Log') && page.includes('Mở danh sách công việc của lớp'), 'The class configuration dialog must explain its purpose, save behaviour and next navigation step.');
assert(page.includes('pendingTaskDisable') && page.includes('Tắt công việc') && page.includes("status: 'CANCELLED'") && page.includes('class-task-name-button'), 'Disabling a class task must require confirmation while task names remain actionable.');
assert(page.includes('Thêm khóa học vào {selectedProject?.code}') && page.includes('inline-course-form') && page.includes('class-detail-modal') && !page.includes('<CourseControlPanel'), 'Course creation must stay inline while class details continue to open in a focused dialog.');
...
assert(page.includes('Mức độ sẵn sàng đầu vào theo từng khóa') && page.includes('getCourseInputProgress') && page.includes('đầu vào sẵn sàng'), 'The input workspace must start with a course list and show server-aggregated progress for each course.');
assert(page.includes("onClick={() => setTab('inputs')}>Đầu vào") && page.includes("activeCourse?.name, 'ĐẦU VÀO'") && !page.includes("'Input readiness'"), 'The input navigation and breadcrumb must use the Vietnamese Đầu vào label.');
...
assert(page.includes('sale-course-list') && page.includes('Lịch lớp') && page.includes('đầu vào sẵn sàng') && page.includes('Xem theo lớp'), 'The Sale upload queue must group class schedules and roster readiness by course and support an all-or-one-class filter.');
assert(page.includes('if (role === \'intake\') return <div className="input-workspace-stack"><DetailedInputWorkspace') && page.includes('<SaleRosterInputs') && page.includes('Việc của tôi · cập nhật danh sách lớp'), 'Sale must retain the roster workspace alongside detailed inputs.');
...
assert(page.includes('InputVersionSnapshot') && page.includes('TaskDetailedInputs') && page.includes("if (tab === 'inputs') return true;"), 'Scoped managers and members consume detailed inputs both in the task layer and input workspace.');
assert(!page.includes('Nguồn input hiện có') && !page.includes('Thông tin đã cung cấp'), 'The task input tab must show actionable task data without a duplicate legacy-source section.');
assert(detailedInputUi.includes('function EmptyInputCard') && detailedInputUi.includes("value || 'Chưa khai báo'") && detailedInputUi.includes('<EmptyInputCard task={task} inputKey={schemaKey} data={emptyData}/>'), 'A task without a created input must still show its complete schema and mark each empty value as not declared.');
assert(detailedInputUi.includes("managing && task.status !== 'IN_REVIEW'") && detailedInputUi.includes('Khai báo đầu vào công việc') && domain.includes("code: 'T-101'") && domain.includes("detailInputKey: 'vtrainingClass'"), 'Managers must receive the corresponding entry form, including completed legacy class-information work.');
assert(page.includes("const editable = canExecute && ['IN_PROGRESS', 'REWORK'].includes(task.status)"), 'The execution tab must always show the checklist while limiting edits to the assigned worker.');
assert(page.includes('<option value="all">Tất cả dự án</option>') && page.includes('<option value="all">Tất cả khóa học</option>') && page.includes('<option value="all">Tất cả lớp</option>'), 'Overview filters must default to explicit all-project, all-course and all-class scopes.');
assert(page.includes('taskDeadlineLabel(task)') && page.includes('taskDeadlineIso(task)') && domain.includes('payload.plannedDeadline'), 'Task views and configuration must use a direct persisted deadline date while retaining legacy offset fallback.');
assert(activeTasksView.includes("const canAssign = role === 'manager' && view !== 'due'") && activeTasksView.includes("['QUÁ HẠN'") && !activeTasksView.includes('.slice(0, 7)'), 'The due inbox must show complete overdue/upcoming groups without assignment controls or a seven-item truncation.');
...
assert(page.includes("const canConfigure = ['operations', 'vtraining', 'manager', 'admin'].includes(role)") && domain.includes('assertTaskConfigurationAccess(state, task.courseId, context)'), 'Assigned team managers and admins must be authorized in both UI and domain before maintaining class tasks.');
...
assert(page.includes("writeRoute('overview', { projectId: '', courseId: '', classId: '', taskId: '', scope: '' }, { replace: true })"), 'Switching roles must clear stale project, course, class and task filters so an admin sees the complete workspace.');
assert(page.includes('const hasAdminCatalog = TRAINING_ROLE_OPTIONS.every') && page.includes('contextCourseOptions') && page.includes('contextClassOptions'), 'An admin role switch must keep every authorized course and class available in the context selectors.');
assert(page.includes("classItem?.projectId || courseItem?.projectId") && page.includes("classItem?.courseId || activeCourse?.id"), 'Selecting an admin-visible class must also switch to its owning project and course.');
...
assert(page.includes('Minh chứng bắt buộc') && page.includes('disabled={!editable || (!checked && !hasEvidence)}') && domain.includes('Cần minh chứng cho tiêu chí'), 'Each completed checklist criterion must have evidence in both the member interface and domain validation.');
...
assert(taskDetailLayer.includes('detail-layer-backdrop" onMouseDown={requestClose}') && taskDetailLayer.includes('onMouseDown={(event) => event.stopPropagation()}'), 'The task detail dialog must close from its backdrop while clicks inside remain contained.');
assert(page.includes('className="modal-backdrop" onMouseDown={close}') && page.includes('aria-labelledby="create-project-title" onMouseDown={(event) => event.stopPropagation()}'), 'The create-project dialog must close from its backdrop while clicks inside remain contained.');
...
assert(css.includes('.task-input-detail .input-current-version dl{display:grid;grid-template-columns:repeat(2') && !css.includes('.vwork-training-operations .input-current-version dl{display:grid;grid-template-columns:repeat(2'), 'Two-column input snapshots must stay scoped to task detail and must not collapse uploaded input cards.');
assert(page.includes('function taskOverdueDays') && page.includes("['DONE', 'CANCELLED'].includes(task.status)") && page.includes('Quá hạn {overdueDays} ngày'), 'Past-deadline tasks must expose an independent overdue badge while completed and cancelled tasks remain exempt.');
assert((page.match(/<TaskStatusBadges task=/g) || []).length >= 5 && css.includes('.task-status-stack') && css.includes('.deadline-overdue'), 'Overdue context must remain visible in task lists, calendar preview, and the task detail header.');
assert(page.includes('<ol className="checklist-grid">') && css.includes('.checklist-grid{display:grid;grid-template-columns:1fr') && !css.includes('.checklist-grid{grid-template-columns:repeat(2'), 'Checklist criteria must remain a semantic ordered list in one vertical column at every width.');

...
assert(api.includes('const fullDirectory = canAssignTasks ? await loadVWorkAccountDirectory(auth) : []') && api.includes('const directory = canAssignTasks ? fullDirectory : []'), 'Managers and operations must receive the complete VWork directory for assignment search.');
...
assert(api.includes('loadTeamDirectory(auth)') && api.includes('accounts.filter((item) => item.active && item.roles.length)') && api.includes("roles.includes('manager') ? 'manager' : roles.includes('member') ? 'member' : roles[0]"), 'Server-side assignment validation must preserve active multi-role identities from the VWork directory.');
assert(page.includes('Bạn muốn giao việc cho ai?') && page.includes('vwork-recent-assignees') && page.includes('Tìm theo tên hoặc email') && page.includes('Toàn bộ tài khoản VWork'), 'Assignment must use a searchable popup containing the complete VWork directory.');
...
assert(page.includes("disabled={person.assignable === false}") && page.includes("chosenPerson.assignable === false"), 'Non-active or unlinked VWork accounts must remain visible but cannot receive assignments.');

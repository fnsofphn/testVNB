...
assert(page.includes("return runCommand('SUBMIT_REVIEW'") && page.includes('checklistEvidence: patch.checklistEvidence') && domain.includes('if (hasOutput) submitOutput') && domain.includes('Cần hoàn thành tiêu chí'), 'Review submission must preserve optional output while requiring the complete checklist and criterion evidence.');
...
assert(taskDetailLayer.includes('detail-layer-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) requestClose(); }}'), 'The task detail dialog must close only when the actual backdrop is pressed.');

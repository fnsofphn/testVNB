...
assert(page.includes("if (role === 'intake') return <div className=\"input-workspace-stack\"><SaleRosterInputs") && page.includes('Việc của tôi · cập nhật danh sách lớp'), 'Sale Input must open the roster upload workspace instead of an empty class-task page.');
...
assert(page.includes('Input dùng để thực hiện công việc') && page.includes('InputVersionSnapshot') && page.includes("!['manager', 'member'].includes(role)"), 'Managers and members must consume active input details inside the task drawer without a standalone Input page.');

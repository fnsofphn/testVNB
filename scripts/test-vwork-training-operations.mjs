...
assert(api.includes('canReceiveVWorkTasks') && api.includes("['manager', 'member'].includes(role)") && api.includes('accounts.filter((item) => item.assignable)'), 'Server-side assignment eligibility must depend only on VWork manager/member roles.');
...
assert(!page.includes('person.assignable === false'), 'Eligible VWork manager/member rows must remain directly selectable without PeopleOne profile checks.');

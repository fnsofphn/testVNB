...
export async function restoreActiveTrainingOperationsLoginAccess(activeRole?: string) {
  ...
  }, activeRole) as Promise<{ ok: true; reconciliation: { candidates: number; checked: number; loginAccessReady: number; restored: number; linked: number; profilesCreated: number; rolesReconciled: number; missingAuth: number; failures: { email: string; error: string }[] } }>;
}

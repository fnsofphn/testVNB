...
export default async function handler(req, res) {
  ...
  if (!await enforceRateLimit(req, res, { route: 'workflow-upload', ...RATE_LIMITS.upload })) return;
  ...
  try {
    ...
    if (!orderId || !productId || !moduleCode || !stageCode || !slot || !fileName) {
    ...
    sendJson(res, 400, { ok: false, error: 'Unsupported upload action.' });
  } catch (error) {

...
export default async function handler(req, res) {
  ...
  if (!await enforceRateLimit(req, res, { route: 'student-survey-delete-submission', ...RATE_LIMITS.mutation })) return;
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey) {
    sendJson(res, 500, { ok: false, error: 'Server delete is not configured.' });
    return;
  }
  ...
  try {
    ...
    if (!submissionId) {
      sendJson(res, 400, { ok: false, error: 'Missing submissionId.' });
      return;
    }

...
- Uploaded-input layout: at the source-matched 1039 × 914 viewport, long MIME types and URLs occupy a 242 px value column and wrap by words/segments rather than one character per line. The file button stays inside its 468 px card; the document width is 1024 px within the 1039 px viewport.
...
12. Uploaded-input pass — P1: a task-detail-only two-column rule leaked into the main input cards. It divided each already narrow metadata row again, collapsing long MIME/URL values into vertical characters and creating extreme card height. Scoped the dense two-column snapshot rule to `.task-input-detail`, restoring the input workspace's single-column metadata list. Also renamed the navigation and breadcrumb to `Đầu vào`. Post-fix capture at 1039 × 914 shows balanced two-column cards, readable metadata, contained file controls, and no horizontal overflow; console errors were empty.
...
- Open `Đầu vào` with an active uploaded Word file; verify long MIME type, URL, and filename remain readable and contained at 1265 px and source-matched 1039 px widths.
